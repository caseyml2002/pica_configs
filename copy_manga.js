    ...
    account = {
        ...
        login: async (account, pwd) => {
            ...
            let res = await Network.post(
                "https://www.mangacopy.com/api/v3/login?platform=3",
                {
                    ...this.headers,
    ...
    explore = [
        {
            ...
            load: async () => {
                let dataStr = await Network.get(
                    "https://www.mangacopy.com/api/v3/h5/homeIndex?platform=3",
                    this.headers
                )
    ...
    categoryComics = {
        load: async (category, param, options, page) => {
            ...
            let res = await Network.get(
                `https://www.mangacopy.com/api/v3/comics?limit=21&offset=${(page - 1) * 21}&ordering=${options[1]}&theme=${param}&top=${options[0]}&platform=3`,
                this.headers
            )
            if (res.status !== 200) {
                throw `Invalid status code: ${res.status}`
    ...
    search = {
        load: async (keyword, options, page) => {
            ...
            var res = await Network.get(
                `https://www.mangacopy.com/api/v3/search/comic?limit=21&offset=${(page - 1) * 21}&q=${keyword}&q_type=&platform=3`,
                this.headers
            )
            if (res.status !== 200) {
                throw `Invalid status code: ${res.status}`
    ...
    favorites = {
        ...
        addOrDelFavorite: async (comicId, folderId, isAdding) => {
            ...
            let comicData = await Network.get(
                `https://www.mangacopy.com/api/v3/comic2/${comicId}?platform=3`,
                this.headers
            )
            ...
            let res = await Network.post(
                "https://www.mangacopy.com/api/v3/member/collect/comic?platform=3",
                {
                    ...this.headers,
                    "Content-Type": "application/x-www-form-urlencoded;charset=utf-8",
                },
        ...
        loadComics: async (page, folder) => {
            var res = await Network.get(
                `https://www.mangacopy.com/api/v3/member/collect/comics?limit=21&offset=${(page - 1) * 21}&free_type=1&ordering=-datetime_updated&platform=3`,
                this.headers
            )
    ...
    comic = {
        loadInfo: async (id) => {
            async function getChapters(id) {
                var res = await Network.get(
                    `https://www.mangacopy.com/api/v3/comic/${id}/group/default/chapters?limit=500&offset=0&platform=3`,
                    this.headers
                );
                if (res.status !== 200) {
                    throw `Invalid status code: ${res.status}`;
                ...
                if (maxChapter > 500) {
                    ...
                    while (offset < maxChapter) {
                        res = await Network.get(
                            `https://www.mangacopy.com/api/v3/comic/chongjingchengweimofashaonv/group/default/chapters?limit=500&offset=${offset}&platform=3`,
                            this.headers
                        );
                        if (res.status !== 200) {
                            throw `Invalid status code: ${res.status}`;
            ...
            async function getFavoriteStatus(id) {
                let res = await Network.get(`https://www.mangacopy.com/api/v3/comic2/${id}/query?platform=3`, this.headers);
                if (res.status !== 200) {
                    throw `Invalid status code: ${res.status}`;
                }
            ...
            let results = await Promise.all([
                Network.get(
                    `https://www.mangacopy.com/api/v3/comic2/${id}?platform=3`,
                    this.headers
                ),
        ...
        loadEp: async (comicId, epId) => {
            let res = await Network.get(
                `https://www.mangacopy.com/api/v3/comic/${comicId}/chapter2/${epId}?platform=3`,
                this.headers
            );
        ...
        loadComments: async (comicId, subId, page, replyTo) => {
            let url = `https://www.mangacopy.com/api/v3/comments?comic_id=${subId}&limit=20&offset=${(page-1)*20}`;
            if(replyTo){
        ...
        sendComment: async (comicId, subId, content, replyTo) => {
            ...
            let res = await Network.post(
                `https://www.mangacopy.com/api/v3/member/comment`,
                {
                    ...this.headers,
                    "Content-Type": "application/x-www-form-urlencoded;charset=utf-8",
                },
