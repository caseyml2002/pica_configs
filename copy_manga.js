class CopyManga extends ComicSource {
    ...
    url = "https://raw.githubusercontent.com/caseyml2002/pica_configs/refs/heads/master/copy_manga.js"
    ...
    account = {
        ...
        login: async (account, pwd) => {
            ...
            let res = await Network.post(
                "https://api.copy-manga.com/api/v3/login?platform=3",
                {
                    ...this.headers,
        ...
        registerWebsite: "https://api.copy-manga.com/web/login/loginByAccount"
    }
    ...
    explore = [
        {
            ...
            load: async () => {
                let dataStr = await Network.get(
                    "https://api.copy-manga.com/api/v3/h5/homeIndex?platform=3",
                    this.headers
                )
        ...
        parts: [
            {
                ...
                categories: ["全部", "爱情", "欢乐向", "冒险", "奇幻", "百合", "校园", "科幻", "东方", "耽美", "生活", "格斗", "轻小说", "悬疑",
                    "其它", "神鬼", "职场", "TL", "萌系", "治愈", "长条","节操","性转换","重生","無修正","後宮"," 异世界","战争","历史","机战","魔幻","竞技","搞笑","励志","彩色"],
                itemType: "category",
                categoryParams: ["", "aiqing", "huanlexiang", "maoxian", "qihuan", "baihe", "xiaoyuan", "kehuan", "dongfang", "danmei", "shenghuo", "gedou", "qingxiaoshuo", "xuanyi",
                    "qita", "shengui", "zhichang", "teenslove", "mengxi", "zhiyu", "changtiao","jiecao","xingzhuanhuan","chongsheng","Uncensored","hougong","yishijie","zhanzheng","lishi","jizhan","mohuan","jingji","gaoxiao","lizhi","COLOR"]
            }
        ]
    }
    ...
    categoryComics = {
        load: async (category, param, options, page) => {
            ...
            let res = await Network.get(
                `https://api.copy-manga.com/api/v3/comics?limit=21&offset=${(page - 1) * 21}&ordering=${options[1]}&theme=${param}&top=${options[0]}&platform=3`,
                this.headers
            )
            if (res.status !== 200) {
                throw `Invalid status code: ${res.status}`
    ...
    search = {
        load: async (keyword, options, page) => {
            ...
            var res = await Network.get(
                `https://api.copy-manga.com/api/v3/search/comic?limit=21&offset=${(page - 1) * 21}&q=${keyword}&q_type=&platform=3`,
                this.headers
            )
            if (res.status !== 200) {
                throw `Invalid status code: ${res.status}`
    ...
    favorites = {
        ...
        addOrDelFavorite: async (comicId, folderId, isAdding) => {
            ...
            let comicData = await Network.get(
                `https://api.copy-manga.com/api/v3/comic2/${comicId}?platform=3`,
                this.headers
            )
            ...
            let res = await Network.post(
                "https://api.copy-manga.com/api/v3/member/collect/comic?platform=3",
                {
                    ...this.headers,
                    "Content-Type": "application/x-www-form-urlencoded;charset=utf-8",
                },
        ...
        loadComics: async (page, folder) => {
            var res = await Network.get(
                `https://api.copy-manga.com/api/v3/member/collect/comics?limit=21&offset=${(page - 1) * 21}&free_type=1&ordering=-datetime_updated&platform=3`,
                this.headers
            )
    ...
    comic = {
        loadInfo: async (id) => {
            async function getChapters(id) {
                var res = await Network.get(
                    `https://api.copy-manga.com/api/v3/comic/${id}/group/default/chapters?limit=500&offset=0&platform=3`,
                    this.headers
                );
                if (res.status !== 200) {
                    throw `Invalid status code: ${res.status}`;
                ...
                if (maxChapter > 500) {
                    ...
                    while (offset < maxChapter) {
                        res = await Network.get(
                            `https://api.copy-manga.com/api/v3/comic/chongjingchengweimofashaonv/group/default/chapters?limit=500&offset=${offset}&platform=3`,
                            this.headers
                        );
                        if (res.status !== 200) {
                            throw `Invalid status code: ${res.status}`;
            ...
            async function getFavoriteStatus(id) {
                let res = await Network.get(`https://api.copy-manga.com/api/v3/comic2/${id}/query?platform=3`, this.headers);
                if (res.status !== 200) {
                    throw `Invalid status code: ${res.status}`;
                }
            ...
            let results = await Promise.all([
                Network.get(
                    `https://api.copy-manga.com/api/v3/comic2/${id}?platform=3`,
                    this.headers
                ),
        ...
        loadEp: async (comicId, epId) => {
            let res = await Network.get(
                `https://api.copy-manga.com/api/v3/comic/${comicId}/chapter2/${epId}?platform=3`,
                this.headers
            );
        ...
        loadComments: async (comicId, subId, page, replyTo) => {
            let url = `https://api.copy-manga.com/api/v3/comments?comic_id=${subId}&limit=20&offset=${(page-1)*20}`;
            if(replyTo){
        ...
        sendComment: async (comicId, subId, content, replyTo) => {
            ...
            let res = await Network.post(
                `https://api.copy-manga.com/api/v3/member/comment`,
                {
                    ...this.headers,
                    "Content-Type": "application/x-www-form-urlencoded;charset=utf-8",
                },
